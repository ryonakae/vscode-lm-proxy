...
export class LimitsManager {
    ...
    try {
      ...
      if (models && models.length > 0) {
        ...
        if ('maxInputTokens' in model) {
          ...
          logger.info(
            `Model ${modelId} token limit retrieved from maxInputTokens: ${limit}`,
          )
          return limit
        }
      }
      ...
      logger.warn(
        `Could not get token limit from API for model ${modelId}, using fallback values`,
      )
      return this.getFallbackTokenLimit(modelId)
    } catch (error) {
      // エラーが発生した場合はログを残し、フォールバック値を返す
      logger.error(
        `Error getting token limit from API: ${(error as Error).message}`,
        error as Error,
      )
      return this.getFallbackTokenLimit(modelId)
    }
   ...
   */
  public async validateTokenLimit(
    messages: any[],
    modelId: string,
  ): Promise<Error | null> {
    // 正確なトークン数を計算
  ...
  public async countTokens(messages: any[], modelId: string): Promise<number> {
    try {
      ...
      logger.error(
        `Error counting tokens: ${(error as Error).message}`,
        error as Error,
      )
      return this.estimateTokenCount(messages)
    }
